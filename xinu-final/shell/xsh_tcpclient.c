...
/*############# Codigo para hacer peticion GET a URL con Protocolo TCP #############*/
...
shellcmd xsh_tcpclient(int nargs, char *args[])
{
    ...
    while (1) {
        ...
        if (bytesRead == 0) {
            ...
            processAndPrintData(msg, bytesRead);
        }
    }
...
void processAndPrintData(unsigned char *data, int32 length)
{
    // Iterar a través de los datos y procesar
    for (int i = 0; i < length; i++) {
        // Omitir etiquetas HTML
        if (data[i] == '<') {
            while (i < length && data[i] != '>')
                i++;
        } else {
            // Mostrar solo el texto fuera de las etiquetas
            printf("%c", data[i]);
        }
    }
}

/*############# FIN codigo para hacer peticion GET a URL #############*/
